...
const LearningQuests: React.FC = () => {
  ...
  const [unlockedLevels, setUnlockedLevels] = useState<{HTML: number, C: number}>({HTML: 1, C: 1});
  ...
  const handlePrevQuest = () => {
    if (!selectedQuest || selectedQuest.id === 1) return;
    const prevId = selectedQuest.id - 1;
    const prevQuest = (activePath === 'HTML' ? HTML_QUESTS : C_QUESTS).find(q => q.id === prevId);
    if (prevQuest) {
      setSelectedQuest(prevQuest);
      setFeedback({ status: null, msg: '' });
    }
  };
  ...
  return (
    <section id="quests" className="py-24 bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div className="space-y-3">
            ...
            <h3 className="text-5xl font-black text-white italic tracking-tighter uppercase">Learning <span className="text-blue-600">Practice</span> Map</h3>
          </div>
          ...
          <div className="flex flex-wrap gap-4 items-center">
             <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-1.5 flex gap-1 backdrop-blur-md">
                {(['HTML', 'C'] as const).map(p => (
                  <button key={p} onClick={() => setActivePath(p)} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activePath === p ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/20' : 'text-slate-500'}`}>{p}</button>
                ))}
             </div>
             <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-1.5 flex gap-1 backdrop-blur-md">
                {(['All', 'Beginner', 'Intermediate', 'Pro'] as const).map(f => (
                  <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-slate-700 text-white' : 'text-slate-600'}`}>{f}</button>
                ))}
             </div>
             <div className="glass-card px-8 py-3 rounded-2xl border-blue-500/20 glow-blue">
                <span className="text-[10px] font-black text-blue-500 mr-4 uppercase tracking-widest">Global XP</span>
                <span className="text-3xl font-black text-white mono">{totalXP}</span>
             </div>
          </div>
        </div>
        ...
        <div className="relative pt-12 pb-24">
          ...
          <div ref={scrollRef} className="quest-scroll flex overflow-x-auto gap-10 pb-12 px-6 scroll-smooth snap-x relative z-10">
            {quests.map((q) => {
              ...
              return (
                <div key={q.id} onClick={() => !isLocked && setSelectedQuest(q)} className={`flex-shrink-0 w-80 snap-center group cursor-pointer transition-all duration-700 ${isLocked ? 'opacity-25 grayscale pointer-events-none' : 'hover:-translate-y-5'}`}>
                  <div className={`h-full glass-card rounded-[50px] border-2 p-10 flex flex-col transition-all relative overflow-hidden ${isCurrent ? 'border-blue-500 shadow-3xl scale-105 bg-blue-600/5' : isCompleted ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-white/5'}`}>
                    ...
                    <h4 className="text-2xl font-black text-white uppercase italic tracking-tighter mb-8 leading-none">{q.title}</h4>

                    <div className="space-y-6 mt-auto">
                      <div className="space-y-2">
                        <p className="text-[9px] font-black text-blue-500 uppercase tracking-widest">Knowledge Insight</p>
                        <p className="text-sm font-bold text-slate-200 leading-tight">{q.knowledgeInsight}</p>
                      </div>
                      <div className="space-y-2">
                        <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Output Insight</p>
                        <p className="text-sm font-bold text-slate-200 leading-tight">{q.outputInsight}</p>
                      </div>
                    </div>
      ...
      {/* Evolution Modal */}
      {selectedQuest && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-slate-950/98 backdrop-blur-3xl animate-in zoom-in-95 duration-500">
          <div className="glass-card w-full max-w-6xl rounded-[60px] border-blue-600/30 flex flex-col h-[90vh] overflow-hidden shadow-4xl relative">
            
            <div className="p-10 border-b border-white/5 flex justify-between items-center bg-slate-900/40">
              <div className="space-y-2">
                <div className="flex items-center space-x-3">
                  <span className="bg-blue-600 text-white px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-[0.2em]">Node {selectedQuest.id}</span>
                  <span className="text-slate-500 font-bold text-[10px] uppercase tracking-widest">/ Path: {activePath}</span>
                </div>
                <h3 className="text-4xl font-black text-white italic uppercase tracking-tighter">{selectedQuest.title}</h3>
              </div>
              <button onClick={() => setSelectedQuest(null)} className="p-5 hover:bg-white/5 rounded-full transition-all text-slate-500 hover:text-white">
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
              <div className="w-full lg:w-[35%] p-10 overflow-y-auto space-y-12 border-r border-white/5 bg-slate-900/20">
                <div className="space-y-4">
                  <h4 className="text-blue-400 font-black text-[10px] uppercase tracking-widest">Problem Statement</h4>
                  <p className="text-2xl font-bold text-white leading-tight">{selectedQuest.problem}</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                    <button 
                      ...
                      className={`flex items-center justify-between p-6 bg-white/5 border rounded-[35px] transition-all text-left ${isSolutionMode ? 'border-blue-500/80 bg-blue-500/5' : 'border-white/10 hover:border-blue-500/50'}`}
                    >
                      <div>
                        <p className="text-[11px] font-black text-white uppercase mb-1">Load Full Solution</p>
                        <p className="text-[10px] text-slate-500 font-medium">Injects complete code structure</p>
                      </div>
                      <svg className="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth="2"/></svg>
                    </button>
                    <button 
                      onClick={() => {setUserCode(selectedQuest.boilerplate); setIsSolutionMode(false); setFeedback({status: null, msg: ''})}} 
                      className={`flex items-center justify-between p-6 bg-white/5 border rounded-[35px] transition-all text-left ${!isSolutionMode ? 'border-emerald-500/80 bg-emerald-500/5' : 'border-white/10 hover:border-emerald-500/50'}`}
                    >
                      <div>
                        <p className="text-[11px] font-black text-white uppercase mb-1">{t('quest.tryByYourself')}</p>
                        <p className="text-[10px] text-slate-500 font-medium">Reset to basic structure</p>
                      </div>
                      <svg className="w-6 h-6 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" strokeWidth="2"/></svg>
                    </button>
                </div>

                {feedback.msg && (
                  <div className={`p-8 rounded-[40px] border animate-in slide-in-from-bottom-5 ${
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest mb-4 flex items-center">
                      <span className={`w-2 h-2 rounded-full mr-3 ${
                        feedback.status === 'success' ? 'bg-emerald-500 shadow-xl shadow-emerald-500/30' : 
                        feedback.status === 'error' ? 'bg-rose-500 animate-pulse' : 
                        'bg-blue-500 animate-bounce'
                      }`}></span>
                      {feedback.status === 'success' ? 'Logic Valid' : feedback.status === 'error' ? 'Logic Fault' : 'Analyzing Logic'}
                    </p>
              ...
              <div className="flex-1 flex flex-col p-10 bg-black/40">
                <div className="relative flex-1 group">
                   <div className="absolute top-0 right-0 p-6 mono text-[10px] text-slate-700 select-none uppercase tracking-widest font-black">Evolution Lab Terminal</div>
                   <textarea 
                    value={userCode}
                    onChange={(e) => !isSolutionMode && setUserCode(e.target.value)}
                    readOnly={isSolutionMode}
                    className={`w-full h-full bg-transparent border-none text-blue-400 mono text-2xl p-0 outline-none resize-none leading-relaxed placeholder:opacity-20 font-medium ${isSolutionMode ? 'opacity-80' : ''}`}
                    spellCheck="false"
                   />
                </div>
                ...
                <div className="pt-10 flex gap-6">
                   {isSolutionMode ? (
                     <button 
                      ...
                      className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-8 rounded-[40px] font-black uppercase tracking-widest text-sm transition-all shadow-3xl shadow-blue-600/40 active:scale-95"
                     >
                     ...
                     <>
                       {feedback.status === 'success' ? (
                         <button 
                          ...
                          className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white py-8 rounded-[40px] font-black uppercase tracking-widest text-sm transition-all shadow-3xl shadow-emerald-600/40 active:scale-95 flex items-center justify-center space-x-4"
                         >
                           <span>{t('quest.next')}</span>
                           <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M13 5l7 7-7 7M5 5l7 7-7 7" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                         </button>
                       ) : (
                         <>
                            <button 
                              onClick={handlePrevQuest} 
                              disabled={selectedQuest.id === 1}
                              className="px-12 py-8 bg-white/5 border border-white/10 text-white rounded-[40px] font-black uppercase tracking-widest text-[11px] hover:bg-white/10 transition-all disabled:opacity-20"
                            >
                              Previous
                            </button>
                            <button 
                              ...
                              className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-8 rounded-[40px] font-black uppercase tracking-widest text-sm transition-all shadow-3xl shadow-blue-600/40 active:scale-95 disabled:opacity-50"
                            >
                              {isVerifying ? t('quest.validating') : `Submit Logic Packet • ${selectedQuest.xp} XP`}
                            </button>
                         </>
                       )}
                     </>
